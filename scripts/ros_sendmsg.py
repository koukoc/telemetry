...
import random
import asyncio
import ros_rocketclass as rocClass
...
class Data:
    ...
    def __init__(self):
        self.FCUSencor = rocClass.FCUSensor()
        self.FlightMode = rocClass.FlightMode()
        # self.NavagationData = rocClass.NavagationData()
        self.PressureSensorData = rocClass.PressureSensorData()
        self.SystemState = rocClass.SystemState()
        self.TVCData = rocClass.TVCData()
    ...
    def SystemStatePacket(self):
        systemState = self.Bool2Byte(self.SystemState.getData())
    ...
    def FlightModePacket(self):
        flightmode = self.FlightMode.getData()
        data = struct.pack('1h', flightmode)
        Mtime = self.MissionTime()
        checksum = self.calculate_checksum(data)
    ...
    def FCUStatePacket(self):
        sensor_state = self.Bool2Byte(self.FCUSencor.getData())
    ...
    def NaviDataPacket(self):
        navi = [self.NavagationData.getData()]
    ...
    def TVCDataPacket(self):
        tvc = self.TVCData.getData()
        data = struct.pack('2h', *tvc)
        Mtime = self.MissionTime()
        checksum = self.calculate_checksum(data)
    ...
    def PressureDataPacket(self):
        pressure = self.PressureSensorData.getData()
...
async def main():
    ...
    await asyncio.gather(task1, task2, task3, task5, task6)
